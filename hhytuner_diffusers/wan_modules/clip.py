...
from wan_modules.xlm_roberta import XLMRoberta
...
def clip_xlm_roberta_vit_h_14(pretrained=False, pretrained_name="open-clip-xlm-roberta-large-vit-huge-14", return_transforms=False, **kwargs):
    cfg = dict(
        embed_dim=1024,
        image_size=224,
        patch_size=14,
...
class CLIPModel:
    def __init__(self, dtype, device, checkpoint_path=None, tokenizer_path=None, weight_path=None):
        ...
        if os.path.splitext(weight_path)[-1] == ".safetensors":
            sd = load_file(weight_path, device=str(device))
        else:
