...
from wan_modules.fp8_optimization_utils import apply_fp8_monkey_patch
from qwen_modules.attention import attention as hunyuan_attention
...
class QwenImageTransformer2DModel(nn.Module):  # ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    ...
    def enable_gradient_checkpointing(self, activation_cpu_offloading: bool = False):
        ...
        print(f"QwenModel: Gradient checkpointing 已启用, Activation CPU offloading: {activation_cpu_offloading}")

    def disable_gradient_checkpointing(self):
        self.gradient_checkpointing = False
        self.activation_cpu_offloading = False
        print("QwenModel: Gradient checkpointing 已禁用")
    ...
    def enable_block_swap(
        ...
        print(
            f"QwenModel: Block swap 已启用, 交换 {self.blocks_to_swap}/{self.num_blocks} 个 blocks, 支持反向传播: {supports_backward}, Pinned memory: {use_pinned_memory}"
        )
    ...
    def switch_block_swap_for_inference(self):
        if self.blocks_to_swap:
            ...
            print("QwenModel: Block swap 设置为仅前向")

    def switch_block_swap_for_training(self):
        if self.blocks_to_swap:
            self.offloader.set_forward_only(False)
            self.prepare_block_swap_before_forward()
            print("QwenModel: Block swap 设置为前向和反向")
    ...
    def save_pretrained(self, save_directory: str, safe_serialization: bool = True):
        ...
        config = {
            ...
            "num_layers": self.config.get("num_layers", len(self.transformer_blocks)),
            "attention_head_dim": self.config.get("attention_head_dim", 128),
            "num_attention_heads": self.config.get("num_attention_heads", 24),
            "joint_attention_dim": self.config.get("joint_attention_dim", 3584),
            "guidance_embeds": self.config.get("guidance_embeds", False),
            "axes_dims_rope": list(self.config.get("axes_dims_rope", [16, 56, 56])),
            "zero_cond_t": getattr(self, "zero_cond_t", False),
        }
...
def load_qwen_transformer_from_diffusers(
    model_id: str,
    ...
    use_fp8: bool = False,
    attn_mode: str = "torch",
    split_attn: bool = True,
) -> "QwenImageTransformer2DModel":
    import json
    import os
    from pathlib import Path
    from safetensors import safe_open
    from wan_modules.fp8_optimization_utils import optimize_state_dict_with_fp8
    ...
    use_layer3d_rope = config.get("use_layer3d_rope", False)
    num_layers = config.get("num_layers", 60)
    ...
    if use_fp8:
        calc_device = device if device is not None else torch.device("cuda")
        ...
        logger.info("QwenImage: Loaded with FP8 optimization")
    else:
        for key in state_dict:
    ...
    if device is not None and not use_fp8:
        model.to(device)
